...
3. 執行基本面分析（上市個股）或 ETF 基本資料查詢（ETF 代碼），僅供價值面參考，不納入技術分數或 ML 特徵
4. 取得近期公開新聞輿情摘要（不納入技術分數或 ML 特徵）
5. 僅在使用者明確指定 `--market-code` 時，載入對應基準資料作跨資產特徵
6. 載入 `models/<code>_best_params.json`（若存在）作 Optuna 調過的最佳參數
7. 用 XGB+LGB ensemble 預測未來 N 天（含籌碼特徵）
8. 給出 BUY/HOLD/SELL 訊號 + 建議止損價（現價 × 0.95）
