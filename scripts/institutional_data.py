...
def load_institutional_df(code: str, data_dir: str = "data") -> Optional[pd.DataFrame]:
    """載入已存在的籌碼資料 CSV
...
def main():
    ...
    if args.action == "fetch":
        ...
        print(f"開始抓取 {args.code} 的籌碼資料（{args.days} 天）...")
        ...
        print(f"取得 {len(df)} 筆籌碼資料 ({df['date'].min().date()} ~ {df['date'].max().date()})")

        # 集保分散表（週頻）
        tdcc = fetcher.fetch_tdcc_concentration(args.code)
        if tdcc:
            print(f"集保大戶持股比例: {tdcc['concentration_pct']}%")
        ...
        if df is None:
            # 嘗試自動抓取
            print(f"未找到 {args.code} 籌碼資料，自動抓取中...")
            fetcher = InstitutionalFetcher()
            df = fetcher.fetch_history(args.code, args.days)
            if df.empty:
                result = {"error": f"無法取得 {args.code} 的籌碼資料"}
